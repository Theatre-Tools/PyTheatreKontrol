...
class VersionValidator(BaseModel):
    ...
    @property
    def eos(self) -> ParsedVersion:
        if self.args and len(self.args) >= 2:
            try:
                version_str = self.args[0].value
                return ParsedVersion(version=version_str)
            except (IndexError, AttributeError):
                raise ValueError("Invalid argument for version message. Expected a string.")
        raise ValueError("No arguments provided for version message.")

    @property
    def fixture_lib(self) -> ParsedVersion:
        if self.args and len(self.args) >= 2:
            try:
    ...
    def gel_swatch_type(self) -> int:
        if self.args and len(self.args) >= 3:
            try:
                ...
                raise ValueError("Invalid argument for version message. Expected an integer.")
        raise ValueError("No arguments provided for version message.")
